...
signed_value = _generated_signing_key.sign(f"{_created_timestamp}{json.dumps(_created_message)}".encode(), encoder=HexEncoder)
verification_key = _generated_signing_key.verify_key.encode(encoder=HexEncoder)

app = FastAPI()
app.add_middleware(DiscordVerificationMiddleware, client_public_key=verification_key.decode())	
...
def test_ack_ping_discord():
    response = client.post("/interactions", headers={
        "X-Signature-Ed25519": signed_value.signature.decode(),
        "x-Signature-Timestamp": _created_timestamp
    }, json=_created_message)
